...
def compute_lags(s1, s2, sr, max_dist_m=10000*1000):
...
def compute_recording_lags(r1, r2):
    start_offset = (r1.timestamps[0] - r2.timestamps[0]) / 1e9
    lag_time, sigma2 = compute_lags(
        r1.samples,
        r2.samples,
        np.mean([r1.sr, r2.sr]),
    )
...
def get_dist_probability_fn(lag_time, sigma2):
...
def heatmap_intensity(m):
    mmin = np.min(m)
    mmax = np.max(m)
    return 1.0 - (m - mmin) / (mmax - mmin)


def tdoa_recs(recs, p1, p2, res):
    if len(recs) < 2:
        raise Exception(f"need at least two recordings for TDoA, got {len(recs)}")

    TDoARecording.sync_recs(recs)

    rx_combos = {}

    for a, b in itertools.combinations(range(len(recs)), 2):
        lag_time, sigma2 = compute_recording_lags(recs[a], recs[b])
        rx_combos[(a, b)] = get_dist_probability_fn(lag_time, sigma2)

    latgr, longr, m = prepare_heatmap(p1, p2, res)

    for (a, b), sigma2 in rx_combos.items():
        d1 = tools.haversine(latgr, longr, recs[a].lat, recs[a].lon)
        d2 = tools.haversine(latgr, longr, recs[b].lat, recs[b].lon)

        dist = d1 - d2

        m += sigma2(dist)

    intensity = heatmap_intensity(m)
